...
- **Thông tin máy con**: IP (`client_addr`), phần mềm (`application_name`) tra từ `pg_stat_activity` theo PID; tên máy qua **reverse-DNS** (best-effort — trống nếu LAN không có bản ghi PTR).
